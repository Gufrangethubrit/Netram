import React from 'react'
import Header from '../components/Header';
import Breadcrum from '../components/Breadcrum';
import Footer from '../components/Footer';

export default function Partners() {
  const tilt = (e: React.MouseEvent<HTMLDivElement>, element: HTMLDivElement) => {
    const rect = element.getBoundingClientRect();
    const x = (e.clientX - rect.left) / rect.width;
    const y = (e.clientY - rect.top) / rect.height;
    const tiltX = (y - 0.5) * 10;
    const tiltY = (0.5 - x) * 10;
    element.style.transform = `rotateX(${tiltX}deg) rotateY(${tiltY}deg)`;
  };

  const untilt = (element: HTMLDivElement) => {
    element.style.transform = 'rotateX(0) rotateY(0)';
  };

  return (
    <div>
      <Header />
      <Breadcrum title="Our Supporting Partners" />

      <section
        className="compliance-section section-padding-sm"
        id="compliance"
      >
        <div className="container">
          <h2 className="section-title text-center">
            Our Supporting <span className="gradient-text">Partners</span>
          </h2>
          <div className="row g-4 justify-content-center partner-grid">
            {/* Loop for all 28 partners */}
            {/* ... Repeat for 2 to 28 */}
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/2.png"
                  alt="Partner 2"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/3.png"
                  alt="Partner 3"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/4.png"
                  alt="Partner 4"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/5.png"
                  alt="Partner 5"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/6.png"
                  alt="Partner 6"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/7.png"
                  alt="Partner 7"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/8.png"
                  alt="Partner 8"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/9.png"
                  alt="Partner 9"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/10.png"
                  alt="Partner 10"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/11.png"
                  alt="Partner 11"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/12.png"
                  alt="Partner 12"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/13.png"
                  alt="Partner 13"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/14.png"
                  alt="Partner 14"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/15.png"
                  alt="Partner 15"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/16.png"
                  alt="Partner 16"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/17.png"
                  alt="Partner 17"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/18.png"
                  alt="Partner 18"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/19.png"
                  alt="Partner 19"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/20.png"
                  alt="Partner 20"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/21.png"
                  alt="Partner 21"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/22.png"
                  alt="Partner 22"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/23.png"
                  alt="Partner 23"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/24.png"
                  alt="Partner 24"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/25.png"
                  alt="Partner 25"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/26.png"
                  alt="Partner 26"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/27.png"
                  alt="Partner 27"
                  loading="lazy"
                />
              </div>
            </div>
            <div className="col-6 col-md-4 col-lg-3">
              <div className="compliance-card partner-card h-100">
                <img
                  className="partner-card-image"
                  src="./assets/partners/28.png"
                  alt="Partner 28"
                  loading="lazy"
                />
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* ----branch----- */}
      <div className="dashboard">
        {/* Cards */}
        <div className="cards-col">
          <div
            className="stat-card card-purple"
onMouseMove={(e) => tilt(e, e.currentTarget)}
             onMouseLeave={(e) => untilt(e.currentTarget)}
          >
            <div className="icon-wrap icon-purple">
              <svg viewBox="0 0 24 24">
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
              </svg>
            </div>
            <div className="card-info">
              <div className="card-num" id="n0">
                36
              </div>
              <div className="card-label">States</div>
              <div className="card-sub">Across the Country</div>
            </div>
          </div>
          <div
            className="stat-card card-blue"
onMouseMove={(e) => tilt(e, e.currentTarget)}
             onMouseLeave={(e) => untilt(e.currentTarget)}
          >
            <div className="icon-wrap icon-blue">
              <svg viewBox="0 0 24 24">
                <path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z" />
              </svg>
            </div>
            <div className="card-info">
              <div className="card-num" id="n1">
                10
              </div>
              <div className="card-label">Our Members</div>
              <div className="card-sub">Across the Country</div>
            </div>
          </div>
          <div
            className="stat-card card-amber"
onMouseMove={(e) => tilt(e, e.currentTarget)}
             onMouseLeave={(e) => untilt(e.currentTarget)}
          >
            <div className="icon-wrap icon-amber">
              <svg viewBox="0 0 24 24">
                <path d="M20.5 3l-.16.03L15 5.1 9 3 3.36 4.9c-.21.07-.36.25-.36.48V20.5c0 .28.22.5.5.5l.16-.03L9 18.9l6 2.1 5.64-1.9c.21-.07.36-.25.36-.48V3.5c0-.28-.22-.5-.5-.5zM15 19l-6-2.11V5l6 2.11V19z" />
              </svg>
            </div>
            <div className="card-info">
              <div className="card-num" id="n2">
                1
              </div>
              <div className="card-label">Our Branches</div>
              <div className="card-sub">Across the Country</div>
            </div>
          </div>
          <div
            className="stat-card card-teal"
onMouseMove={(e) => tilt(e, e.currentTarget)}
             onMouseLeave={(e) => untilt(e.currentTarget)}
          >
            <div className="icon-wrap icon-teal">
              <svg viewBox="0 0 24 24">
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
              </svg>
            </div>
            <div className="card-info">
              <div className="card-num" id="n3">
                0
              </div>
              <div className="card-label">Our Volunteers</div>
              <div className="card-sub">Across the Country</div>
            </div>
          </div>
        </div>
        {/* Map */}
        <div className="map-col">
          <div id="map" />
          <div className="map-info-card">
            <div className="map-info-name">
              <i className="fa-solid fa-building" /> Netram Eye Foundation
            </div>
            <div className="map-info-addr">
              <i className="fa-solid fa-location-dot" /> E-98, Greater Kailash-2
              <br />
              <i className="fa-solid fa-map-marker-alt" /> New Delhi – 110048
              <br />
              <i className="fa-solid fa-phone" /> 011-41676655 / 9212646655
            </div>
            <div className="map-info-tags">
              <span className="tag tag-green">
                <i className="fa-solid fa-hand-holding-heart" /> NGO
              </span>
              <span className="tag tag-blue">
                <i className="fa-solid fa-book-open" /> Education
              </span>
              <span className="tag tag-orange">
                <i className="fa-solid fa-heart-pulse" /> Healthcare
              </span>
            </div>
          </div>
          <div className="map-live">
            <div className="live-dot" />
            <i className="fa-solid fa-signal" /> 1 Branch Active
          </div>
        </div>
      </div>

      <Footer />
    </div>
  );
}
